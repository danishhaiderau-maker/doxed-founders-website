...
The cloud agent image used for this pass did not include OWASP ZAP, Burp Suite, Docker, or `zap-baseline.py`; only Java and curl were available. Because of that, this pass used non-invasive live header checks and repository review instead of an authenticated ZAP/Burp crawl.
