...
export class FounderNodeService {
  ...
  async validateNodeToken(nodeId: string, nodeToken: string) {
    ...
    if (node.tokenExpiresAt && node.tokenExpiresAt.getTime() <= Date.now()) {
      throw new UnauthorizedException('Founder Node token expired');
    }
    return node;
  }
  ...
  async createDeviceCode(
    ...
    const ipcSecretHash = opts?.ipcSecret
      ? await bcrypt.hash(opts.ipcSecret, 10)
      : null;
  ...
  async pollDeviceCode(
    ...
    const consumed = await this.prisma.founderNodeDeviceCode.updateMany({
      where: {
        id: row.id,
        status: 'authorized',
    ...
    if (consumed.count !== 1) {
      return {
        status: 'expired',
        error: 'Authorized device code was already consumed — start a new sign-in',
      };
    }
  ...
  async authorizeDeviceCode(
    ...
    const node = await this.prisma.$transaction(async (tx) => {
      const claimed = await tx.founderNodeDeviceCode.updateMany({
        where: { id: row.id, status: 'pending' },
        data: { status: 'authorizing', userId },
      });
      if (claimed.count !== 1) {
      ...
      const pairedNode = await tx.founderNode.upsert({
        ...
        create: {
          ...
          ipcSecretHash: row.ipcSecretHash,
        },
        update: {
          userId,
          label: row.deviceLabel?.trim() || `Founder IDE ${nodeId.slice(-8)}`,
      ...
      await tx.founderNodeDeviceCode.update({
        where: { id: row.id },
        data: {
          status: 'authorized',
          nodeToken,
  ...
  async denyDeviceCode(userId: string, userCode: string): Promise<{ denied: true }> {
    ...
    const denied = await this.prisma.founderNodeDeviceCode.updateMany({
      where: { id: row.id, status: 'pending' },
      data: { status: 'denied' },
    });
    if (denied.count !== 1) {
