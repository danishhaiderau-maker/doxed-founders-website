...
test('Bitfinex nonce uses ms×10000 scale and catches up after poisoned high-water mark', () => {
  resetBitfinexNonceLanesForTests();
  const t0 = 1_723_886_277_000;
  const lane = { lastNonce: 0n, running: false, mutations: [], reads: [] };
  const n1 = BigInt(allocMonotonicNonce(lane, t0));
  const n2 = BigInt(allocMonotonicNonce(lane, t0));
  assert.equal(BITFINEX_NONCE_MS_SCALE, 10_000n);
  assert.ok(n1 >= BigInt(t0) * BITFINEX_NONCE_MS_SCALE);
  assert.ok(n2 > n1);

  resetBitfinexNonceLanesForTests();
  const poisonedHighWater = BigInt(Date.now()) * BITFINEX_NONCE_MS_SCALE;
  const poisonedLane = {
    lastNonce: poisonedHighWater - 5n,
    running: false,
    mutations: [],
    reads: [],
  };
  const recovered = BigInt(allocMonotonicNonce(poisonedLane));
  assert.ok(recovered > poisonedHighWater - 5n);
  assert.ok(recovered >= BigInt(Date.now()) * BITFINEX_NONCE_MS_SCALE);
});
