  ...
  try {
    res = await fetch(apiUrl('/copilot/ask/stream'), {
      ...
      headers: {
        ...
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
        ...(options?.userApiKey ? { 'X-User-Api-Key': options.userApiKey } : {}),
      },
      body: JSON.stringify({
        prompt,
        agentTemplate: options?.agentTemplate ?? undefined,
...
/** Returns the AI brains the calling user can pick from in the chat dropdown,
 * reflecting which platform promo keys are configured + whether the user's
 * Founder Node is reporting Ollama. RULE_BASED is always included. */
export function fetchAvailableBrains(token: string) {
  return apiFetch<AvailableBrain[]>('/founder-os/available-brains', undefined, token);
}
