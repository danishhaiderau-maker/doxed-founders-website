    ...
    <div className='flex h-[calc(100vh-3.5rem)] w-full bg-[#08080c] text-zinc-100'>
      <aside className='flex w-72 shrink-0 flex-col border-r border-white/5 bg-[#0a0a0f]'>
        ...
        <div className='min-h-0 flex-1 overflow-y-auto px-2 pb-3'>
          {workspaces.length === 0 && <div className='px-3 py-6 text-center text-xs text-zinc-600'>No workspaces yet. Connect an IDE to see your recent sessions.</div>}
          {workspaces.map((w) => (
            <button key={w.id} onClick={() => setSelectedWsId(w.id)} className={'mb-1 block w-full rounded-lg px-3 py-2.5 text-left transition ' + (selectedWsId === w.id ? 'bg-white/10 ring-1 ring-white/15' : 'hover:bg-white/5')}>
              <div className='flex items-center gap-2'>
                <span className={'h-1.5 w-1.5 shrink-0 rounded-full ' + (w.source === 'bridge' ? 'bg-emerald-400' : 'bg-zinc-500')} />
        ...
        <div className='border-t border-white/5 px-2 py-2'>
          <button onClick={() => setShowSocial((v) => !v)} className='mb-1 block w-full rounded-md px-3 py-1.5 text-left text-xs text-zinc-400 hover:bg-white/5 hover:text-zinc-200'>{showSocial ? 'Hide' : 'Show'} social</button>
          <button onClick={() => setShowSettings((v) => !v)} className='block w-full rounded-md px-3 py-1.5 text-left text-xs text-zinc-400 hover:bg-white/5 hover:text-zinc-200'>{showSettings ? 'Hide' : 'Show'} settings</button>
        </div>
      </aside>
      <main className='flex min-w-0 flex-1 flex-col'>
        ...
        {showSocial && socialPanel && <div className='max-h-48 overflow-y-auto border-b border-white/5 bg-[#0a0a0f] px-4 py-2'>{socialPanel}</div>}
        {showSettings && settingsPanel && <div className='max-h-64 overflow-y-auto border-b border-white/5 bg-[#0a0a0f] px-4 py-2'>{settingsPanel}</div>}
