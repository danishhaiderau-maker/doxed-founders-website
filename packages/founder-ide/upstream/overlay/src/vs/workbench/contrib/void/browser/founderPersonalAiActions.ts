...
const GLM_TRANSCRIPTION_ENDPOINT_GLOBAL = 'https://api.z.ai/api/paas/v4/audio/transcriptions';
const GLM_TRANSCRIPTION_ENDPOINT_CHINA = 'https://open.bigmodel.cn/api/paas/v4/audio/transcriptions';
...
const glmTranscriptionEndpoint = (profile: FounderProviderProfile): string => {
	try {
		...
		// Saved profiles are validated before this point; retain the global safe default.
	}
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor, input: FounderPersonalAiTranscriptionInput) {
		...
		try {
			...
			const response = await fetch(glmTranscriptionEndpoint(profile), {
				method: 'POST',
