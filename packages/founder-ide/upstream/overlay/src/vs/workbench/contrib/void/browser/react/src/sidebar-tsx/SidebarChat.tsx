...
import { AlertTriangle, File, Ban, Check, ChevronRight, Dot, FileIcon, Pencil, Undo, Undo2, X, Flag, Copy as CopyIcon, Info, CirclePlus, Ellipsis, CircleEllipsis, Folder, ALargeSmall, TypeOutline, Text, Mic, MicOff, Settings2, Brain, Paperclip, Image as ImageIcon } from 'lucide-react';
...
type FounderVisualDescription = {
	name: string;
	description: string;
};

type FounderBrowserFile = InstanceType<typeof globalThis.File>;

const FOUNDER_VISUAL_TYPES = new Set<FounderVisualAttachment['mimeType']>([
	'image/png',
	'image/jpeg',
	'image/webp',
]);
const FOUNDER_VISUAL_MAX_FILES = 4;
const FOUNDER_VISUAL_MAX_FILE_BYTES = 4 * 1024 * 1024;
const FOUNDER_VISUAL_MAX_TOTAL_BYTES = 7 * 1024 * 1024;

const founderVisualAttachmentId = (file: FounderBrowserFile): string =>
	`${file.name}:${file.size}:${file.lastModified}:${crypto.randomUUID()}`;

const founderVisualBase64 = async (file: FounderBrowserFile): Promise<string> => {
	const buffer = await file.arrayBuffer();
	return founderVoiceBase64(buffer);
};

const founderVisualContext = (
	attachments: FounderVisualAttachment[],
	descriptions: FounderVisualDescription[],
): string => {
	const byName = new Map(descriptions.map(item => [item.name, item.description.trim()]));
	const evidence = attachments.map((attachment, index) => {
		const description = byName.get(attachment.name);
		if (!description) throw new Error(`Founder could not understand ${attachment.name}. Remove it or try again.`);
...
export const SidebarChat = () => {
	...
	const [visualAttachments, setVisualAttachments] = useState<FounderVisualAttachment[]>([])
	const [visualPhase, setVisualPhase] = useState<'idle' | 'processing'>('idle')
	...
	const addVisualFiles = useCallback(async (files: Iterable<FounderBrowserFile>) => {
		const candidates = [...files].filter(file => file.size > 0)
		if (candidates.length === 0) return
		setVisualError(null)
		try {
			const next: FounderVisualAttachment[] = []
			for (const file of candidates) {
				if (!FOUNDER_VISUAL_TYPES.has(file.type as FounderVisualAttachment['mimeType'])) {
					throw new Error('Founder accepts PNG, JPEG, and WebP screenshots.')
				}
				if (file.size > FOUNDER_VISUAL_MAX_FILE_BYTES) {
					throw new Error(`${file.name} is larger than 4 MB.`)
				}
				next.push({
					id: founderVisualAttachmentId(file),
					name: file.name.slice(0, 180),
					mimeType: file.type as FounderVisualAttachment['mimeType'],
					dataBase64: await founderVisualBase64(file),
					size: file.size,
				})
			}
			const combined = [...visualAttachments, ...next]
				.filter((item, index, all) =>
					all.findIndex(candidate =>
						candidate.name === item.name
						&& candidate.size === item.size
						&& candidate.dataBase64 === item.dataBase64
					) === index
				)
			if (combined.length > FOUNDER_VISUAL_MAX_FILES) {
				setVisualError('Founder accepts up to four screenshots per message.')
				return
			}
			const total = combined.reduce((sum, item) => sum + item.size, 0)
			if (total > FOUNDER_VISUAL_MAX_TOTAL_BYTES) {
				setVisualError('Attached screenshots exceed the 7 MB total limit.')
				return
			}
			setVisualAttachments(combined)
		} catch (error) {
			setVisualError(error instanceof Error ? error.message : 'Founder could not attach that image.')
		}
	}, [visualAttachments])

	const onVisualPaste = useCallback((event: React.ClipboardEvent<HTMLDivElement>) => {
	...
	const onVisualDrop = useCallback((event: React.DragEvent<HTMLDivElement>) => {
		...
		try {
			if (!_skipVisualAttachments && visualAttachments.length > 0) {
				...
				const result = await commandService.executeCommand<{ descriptions?: FounderVisualDescription[] }>(
					...
					{
						attachments: visualAttachments.map(({ name, mimeType, dataBase64 }) => ({
							name,
							mimeType,
							dataBase64,
						})),
				...
				const context = founderVisualContext(visualAttachments, result?.descriptions ?? [])
			...
			if (visualAttachments.length > 0) {
				setVisualError(e instanceof Error ? e.message : 'Founder could not understand the attached screenshots.')
			}
			return
		} finally {
			setVisualPhase('idle')
	...
	}, [chatThreadsService, commandService, isDisabled, isRunning, setSelections, visualAttachments, visualPhase, voicePhase])
	...
	>
		...
		{visualAttachments.length > 0 ? <div className='flex min-w-0 gap-1.5 overflow-x-auto pb-0.5' aria-label='Attached screenshots'>
			{visualAttachments.map(attachment => <div
				key={attachment.id}
				className='group relative h-14 w-20 shrink-0 overflow-hidden rounded border border-void-border-2 bg-void-bg-1'
			...
			>
				...
				<button
					type='button'
					className='absolute right-0.5 top-0.5 flex h-5 w-5 items-center justify-center rounded bg-void-bg-1/90 text-void-fg-2 opacity-90 hover:text-void-fg-1'
					title={`Remove ${attachment.name}`}
					aria-label={`Remove ${attachment.name}`}
					onClick={() => setVisualAttachments(current => current.filter(item => item.id !== attachment.id))}
				>
					<X size={13} strokeWidth={2} />
				</button>
				<span className='absolute inset-x-0 bottom-0 truncate bg-void-bg-1/90 px-1 py-0.5 text-[9px] text-void-fg-2'>
					{attachment.name}
				</span>
			</div>)}
		</div> : null}
		...
		rightControls={<>
			...
			<button
				...
				disabled={!!isRunning || visualPhase !== 'idle'}
				onClick={(event) => {
			...
			<button
				type='button'
				className='h-[24px] w-[24px] flex items-center justify-center rounded text-void-fg-3 hover:text-void-fg-1 hover:bg-void-bg-2'
				title='Add or manage personal AI'
			...
			<button
				type='button'
				className={`h-[24px] w-[24px] flex items-center justify-center rounded hover:bg-void-bg-2 ${voicePhase === 'listening' ? 'text-red-400' : 'text-void-fg-3 hover:text-void-fg-1'}`}
				title={voicePhase === 'listening' ? 'Stop voice input' : 'Voice input'}
		...
		statusMessage={visualError
			...
			: visualPhase === 'processing'
				? <div role='status' className='px-1 pb-1 text-[11px] text-void-fg-3'>Reading attached screenshots...</div>
				: voiceError
			? <div role='alert' className='px-1 pb-1 text-[11px] text-amber-400'>{voiceError}</div>
			: voicePhase !== 'idle'
				? <div role='status' className='px-1 pb-1 text-[11px] text-void-fg-3'>{voicePhase === 'listening' ? 'Listening...' : voicePhase === 'processing' ? 'Finishing transcription...' : 'Starting microphone...'}</div>
				: !voiceSupported
					? null
					: null}
	>
	...
	const initiallySuggestedPromptsHTML = <div className='flex flex-col gap-2 w-full text-nowrap text-void-fg-3 select-none'>
		{[
			...
			'Create project instructions for me'
		].map((text, index) => (
			<div
				key={index}
