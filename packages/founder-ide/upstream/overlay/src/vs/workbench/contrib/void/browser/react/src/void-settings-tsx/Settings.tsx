...
const SimpleModelSettingsDialog = ({
	...
	const sourcecodeOverridesLink = 'https://github.com/danishhaiderau-maker/doxedcryptofounder'
		...
		>
			...
			>
				...
				<div className="text-sm text-void-fg-3 mb-4">
					{type === 'default' ? `${modelName} comes packaged with Founder IDE, so you shouldn't need to change these settings.`
						: isUnrecognizedModel
							? `Model not recognized by Founder IDE.`
							: `Founder IDE recognizes ${modelName} ("${recognizedModelName}").`}
				</div>
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`Founder IDE automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			downloadName = 'founder-chats.json'
		}
		else if (t === 'Settings') {
			// Export user settings
			dataStr = JSON.stringify(voidSettingsService.state, null, 2)
			downloadName = 'founder-ai-settings.json'
		}
		else {
			dataStr = ''
			downloadName = ''
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>Personal AI</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Run models privately on this device. Founder IDE detects supported local providers automatically.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`Connect personal providers such as Anthropic, OpenAI, OpenRouter, GLM, DeepSeek, and more.`}</h3>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of Founder AI suggestions in the code editor.`}</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Import compatible editor settings into Founder IDE.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Import or export Founder IDE settings and chats.`}</h4>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Anonymous product metrics help keep Founder IDE reliable. You may opt out below. Founder IDE does not include your code, messages, or API keys in these metrics.</h4>
...
Alternatively, place a \`.voidrules\` compatibility file in the root of your workspace.
								`} chatMessageLocation={undefined} />
									...
									<div className='my-4'>
										...
										<div className='text-void-fg-3 text-xs mt-1'>
											{`When disabled, Founder IDE will not include anything in the system message except for content you specified above.`}
										</div>
									</div>
								</div>
