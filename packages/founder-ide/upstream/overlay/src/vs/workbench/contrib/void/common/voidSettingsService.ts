...
const _validatedModelState = (state: Omit<VoidSettingsState, '_modelOptions'>): VoidSettingsState => {
	...
	for (const providerName of providerNames) {
		...
		const hasFounderManagedModel = providerName === 'openAICompatible'
			&& settingsAtProvider.models.some(model => model.modelName.startsWith('founder-os-'))
		const didFillInProviderSettings = hasFounderManagedModel
...
const defaultState = () => {
	const founderAuto: ModelSelection = { providerName: 'openAICompatible', modelName: 'founder-os-auto' }
	const founderCode: ModelSelection = { providerName: 'openAICompatible', modelName: 'founder-os-code' }
	const d: VoidSettingsState = {
		settingsOfProvider: deepClone(defaultSettingsOfProvider),
		modelSelectionOfFeature: {
			'Chat': deepClone(founderAuto),
			'Ctrl+K': deepClone(founderAuto),
			'Autocomplete': deepClone(founderCode),
			'Apply': deepClone(founderAuto),
			'SCM': deepClone(founderAuto),
		},
