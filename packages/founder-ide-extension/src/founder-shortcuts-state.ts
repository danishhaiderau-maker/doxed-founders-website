export type FounderShortcutSurface =
  | 'agents'
  | 'ship'
  | 'node'
  | 'connections'
...
export interface FounderShortcutState {
  connected: boolean;
  workspaceName: string;
  modeLabel: string;
}

export interface FounderShortcutEntry {
  id: string;
  label: string;
  description?: string;
  icon: string;
...
export const FOUNDER_SHORTCUT_SURFACES: readonly FounderShortcutSurface[] = [
  'agents',
  'ship',
  'node',
  'connections',
...
export const FOUNDER_SHORTCUT_VIEW_IDS: Readonly<
  Record<FounderShortcutSurface, string>
> = {
...
export function shortcutEntries(
  surface: FounderShortcutSurface,
  state: FounderShortcutState,
): readonly FounderShortcutEntry[] {
  switch (surface) {
    case 'agents':
      return [
        {
