...
function standaloneGenomeCountCards(d) {
  ...
  const sourceUnavailable = ['V3_REPORT_NOT_GENERATED', 'REPORT_NOT_IN_CURRENT_GENERATION'].includes(d.status);
  const count = value => sourceUnavailable ? 'UNAVAILABLE' : (value ?? 'UNAVAILABLE');
  return [
    ['Independent episodes', count(c.independent_opportunities)],
    ...
    ['Complete policies evaluated', count(cs.unique_policies_evaluated ?? s.unique_policies_evaluated)],
    ['Nominal search space', count(s.nominal_full_cartesian)],
  ];
}
</script>
