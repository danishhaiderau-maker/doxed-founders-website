...
<script>fetch('/api/cross-world-evidence').then(r=>r.json()).then(d=>{const j=d.join_summary||{},worlds=d.worlds||{};document.getElementById('banner').textContent=(j.status||d.status||'NOT_COMPUTABLE')+' · comparisons '+String(j.pairwise_computable_comparisons??0)+' · disagreements '+String(j.pairwise_disagreements??0);document.getElementById('worlds').innerHTML=Object.entries(worlds).map(([name,w])=>'<div class="card"><small>'+name+'</small><div class="value">'+String(w.status||'NOT_COMPUTABLE')+'</div><div>observed '+String(w.rows_observed??0)+' · complete identity '+String(w.rows_with_complete_explicit_identity??0)+' · unique joinable '+String(w.unique_joinable_rows??0)+'</div></div>').join('')||'<div class="card">No current world inventory is available.</div>';document.getElementById('detail').textContent=JSON.stringify({join_summary:j,worlds:worlds,source_inventory:d.source_inventory},null,2)}).catch(e=>{document.getElementById('banner').textContent='FAILED TO LOAD · '+e});</script></body></html>
""")
