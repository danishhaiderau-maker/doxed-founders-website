...
Output: downloads/chatgpt_research_bundle.zip
        downloads/BUNDLE_MANIFEST.json (human-readable index)
"""
...
AGENT_ROOT = FINAL_BOTS_ROOT / "doxedcryptofounder" / "services" / "btc-conservative-agent"
OUT_DIR = FINAL_BOTS_ROOT / "downloads"
...
def _find_file(name: str) -> Path | None:
    for root in (AGENT_ROOT, FINAL_BOTS_ROOT):
        p = root / name
        if p.is_file():
            return p
    return None
...
def _session_archive_count() -> int:
    arch = FINAL_BOTS_ROOT / "research_session_archives"
    if not arch.is_dir():
        arch = AGENT_ROOT / "research_session_archives"
    if not arch.is_dir():
        return 0
...
def _atomic_zip_build(files: list[tuple[Path, str]], out_zip: Path) -> None:
    """Write ZIP via temp file so central directory is never truncated."""
...
def build() -> tuple[Path, dict]:
    ...
    acc_csv = FINAL_BOTS_ROOT / "research_accumulator" / "trades_accumulated.csv"
    if not acc_csv.is_file():
        acc_csv = AGENT_ROOT / "research_accumulator" / "trades_accumulated.csv"
    if acc_csv.is_file():
        to_pack.append((acc_csv, "accumulator/trades_accumulated.csv"))
        manifest["files_included"].append("accumulator/trades_accumulated.csv")
        manifest["accumulator_trades"] = max(0, sum(1 for _ in acc_csv.open(encoding="utf-8")) - 1)
    ...
    # Timeline from complete bundle if present
    for src_name, arc in (
