...
def verify_generation(
    ...
    *,
    ...
    resolved_source = lexical_root.resolve(strict=True)
    source_is_file = resolved_source.is_file()
    root = resolved_source.parent if source_is_file else resolved_source
    ...
    required_identity = ("source_revision", "deployed_revision", "collection_epoch_id",
    ...
    if (not _REV.fullmatch(str(identity.get("source_revision") or ""))
            ...
            or not _SHA.fullmatch(str(identity.get("tile_registry_signature") or ""))
    ...
    actual = ({resolved_source.name} if source_is_file else {
...
class RawGenerationCleanupTransaction:
    ...
    def quarantine(self, source: Path, proof: Mapping[str, Any], *,
        ...
        return {"status": "QUARANTINED_SOURCE_RETAINED", "generation_id": generation_id,
                ...
                "source_cleanup_authorized": True, "free_bytes_before": free_before,
    ...
    def reconcile(self, generation_id: str | None = None) -> list[dict[str, Any]]:
        ...
        for prepared_path in sorted(self.tx_root.glob("*/PREPARED.json")):
            ...
            if source.exists() and not quarantine.exists():
                ...
                free_after = shutil.disk_usage(self.root).free
                _write_once(tx / "QUARANTINED.json", {
                    **row, "state": "QUARANTINED", "committed_at": datetime.now(timezone.utc).isoformat(),
                    "free_bytes_after": free_after,
                    "free_bytes_delta": free_after - int(row["free_bytes_before"]),
                })
